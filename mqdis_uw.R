#library('Hmisc')
mqdis_uw<-function(q,D,C=diag(2)){
  ...
  if (w1[2]*w2[2]<=0) xmin<-quantile(D[,1],q,type=1)
  else xmin<-(-5)
  ...
  i<-dim(wmax)[1]
  while (i>0) {
    qw<-quantile(wmax[i,1]*D[,1]+wmax[i,2]*D[,2],q,type=1)
  ...
  i<-dim(wmin)[1]
  while (i>0) {
    qw<-quantile(wmin[i,1]*D[,1]+wmin[i,2]*D[,2],q,type=1)
  ...
  #+xlim(c(-5,5))+ylim(c(-5,5))+
    ...
    geom_point(data=D,aes(x=D[,1],y=D[,2]))
}
