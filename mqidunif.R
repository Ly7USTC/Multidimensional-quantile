mqidunif<-function(q,Xlim=c(-5,5),Ylim=c(-5,5),C=diag(2)){
  ### 2-dimensional q-quantile for independent 2-dimensional uniform distribution
  ##C is the points generating cone with default R+.
  P<-matrix(NA,nrow=10000,ncol=2)
  P[,1]<-runif(10000,min=Xlim[1],max=Xlim[2])
  P[,2]<-runif(10000,min=Ylim[1],max=Ylim[2])
  P<-as.data.frame(P)
  lam<-seq(0,1,by=0.01)
  lam<-cbind(1-lam,lam)
  if (is.matrix(C)){
  ...
  if (w1[2]*w2[2]<=0) xmin<-qunif(q,min=Xlim[1],max=Xlim[2])
  else xmin<-(-5)
  ...
  i<-dim(wmax)[1]
  while (i>0) {
    qw<-qunif(q,min=wmax[i,1]*Xlim[1]+wmax[i,2]*Ylim[1],max=wmax[i,1]*Xlim[2]+wmax[i,2]*Ylim[2])
  ...
  i<-dim(wmin)[1]
  while (i>0) {
    qw<-qunif(q,min=wmin[i,1]*Xlim[1]+wmin[i,2]*Ylim[1],max=wmin[i,1]*Xlim[2]+wmin[i,2]*Ylim[2])
